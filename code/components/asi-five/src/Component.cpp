				...
				if (LoadPEFile(it->path(), libraryBuffer))
				{
					...
					if (wcsicmp(it->path().filename().c_str(), L"gears.asi") == 0 || wcsicmp(badFileName.c_str(), L"gears.asi") == 0)
					{
						...
						if (header->e_magic == IMAGE_DOS_SIGNATURE)
						{
							...
							{
								MessageBoxW(NULL, L"This version of the manual transmission plugin ('Gears.asi') is outdated and no longer works with VMP. Please update this plugin to a newer version or delete it.", L"VMP", MB_OK | MB_ICONSTOP);
